...
static const int pnoext2int[] =
    ...
    SEI_CERES, SEI_PALLAS, SEI_JUNO, SEI_VESTA,
};

static int do_fread(void *targ, int size, int count, int corrsize, FILE * fp,
                    int32 fpos, int freord, int fendian, int ifno,
                    char *serr);
...
static int main_planet_bary(double tjd, int ipli, int32 epheflag, int32 iflag,
                            AS_BOOL do_save, double *xp, double *xe,
                            double *xs, double *xm, char *serr);
static int32 swecalc(double tjd, int ipl, int32 iflag, double *x, char *serr);
static int sweplan(double tjd, int ipli, int ifno, int32 iflag,
                   AS_BOOL do_save, double *xp, double *xpe, double *xps,
                   double *xpm, char *serr);
static int swemoon(double tjd, int32 iflag, AS_BOOL do_save, double *xp,
                   char *serr);
static int sweph(double tjd, int ipli, int ifno, int32 iflag, double *xsunb,
                 AS_BOOL do_save, double *xp, char *serr);
...
void FAR PASCAL_CONV swe_set_sid_mode(int32 sid_mode, double t0,
                                      double ayan_t0);
static int app_pos_rest(struct plan_data *pdp, int32 iflag, double *xx,
                        double *x2000, struct epsilon *oe, char *serr);

#ifdef TRACE
static void trace_swe_calc(int param, double tjd, int ipl, int32 iflag,
                           double *xx, char *serr);
static void trace_swe_fixstar(int swtch, char *star, double tjd, int32 iflag,
                              double *xx, char *serr);
static void trace_swe_get_planet_name(int swtch, int ipl, char *s);
#endif

char *FAR PASCAL_CONV
swe_version(char *s)
{
    strcpy(s, SE_VERSION);
    return s;
...
/*  if (iflag & SEFLG_ICRS)
    ...
    if ((iflag & SEFLG_SPEED3) == 0) {
        ...
        sd->ipl = ipl;
        if ((sd->iflgsave =
             swecalc(tjd, ipl, iflag, sd->xsaves, serr)) == ERR)
            goto return_error;
    }
    else {
        /*
...
static int32
swecalc(double tjd, int ipl, int32 iflag, double *x, char *serr)
{
    int i;
   ...
   ******************************************/
        ...
        switch (epheflag) {
            ...
            case SEFLG_JPLEPH:
                retc =
                    jplplan(tjd, ipli, iflag, DO_SAVE, NULL, NULL, NULL,
                            serr);
...
sweph_moon:
                ...
                if (iflag & (SEFLG_HELCTR | SEFLG_BARYCTR | SEFLG_NOABERR))
                    retc =
                        sweplan(tjd, ipli, SEI_FILE_MOON, iflag, DO_SAVE,
...
#else
                retc =
                    sweplan(tjd, ipli, SEI_FILE_MOON, iflag, DO_SAVE, NULL,
                            NULL, NULL, NULL, serr);
#endif
                if (retc == ERR)
                    goto return_error;
...
moshier_moon:
                ...
                retc =
                    swi_moshplan(tjd, SEI_EARTH, DO_SAVE, NULL, NULL, serr);
                 /**/ if (retc == ERR)
                    goto return_error;
                break;
            default:
        ...
        switch (epheflag) {
            ...
            case SEFLG_JPLEPH:
                ...
                if (!swed.jpl_file_is_open) {
                    retc =
                        swi_open_jpl_file(ss, swed.jplfnam, swed.ephepath,
                                          serr);
                    if (retc != OK)
                        goto sweph_sbar;
...
sweph_sbar:
                ...
                retc =
                    sweplan(tjd, SEI_EARTH, SEI_FILE_PLANET, iflag, DO_SAVE,
                            NULL, NULL, NULL, NULL, serr);
#if 1
...
moshier_sbar:
   ...
   ******************************************/
             ...
             || ipl == SE_MERCURY || ipl == SE_VENUS || ipl == SE_MARS
             || ipl == SE_JUPITER || ipl == SE_SATURN || ipl == SE_URANUS
             || ipl == SE_NEPTUNE || ipl == SE_PLUTO || ipl == SE_EARTH) {
   ...
   **********************************************/
        ...
        for (i = 0; i <= 1; i++)
            xp2[3 + i] =
                swe_difrad2n(xp2[i], xp2[3 + i]) / MEAN_NODE_SPEED_INTV;
        xp2[5] = 0;
        ndp->teval = tjd;
        ndp->xflgs = -1;
        /* lighttime etc. */
   ...
   ***********************************************/
    ...
    else if (ipl == SE_CHIRON || ipl == SE_PHOLUS || ipl == SE_CERES    /* Ceres - Vesta */
...
do_asteroid:
        ...
        /* asteroid */
        retc =
            sweph(tjd, ipli_ast, ifno, iflag, psdp->x, DO_SAVE, NULL, serr);
        if (retc == ERR || retc == NOT_AVAILABLE)
            goto return_error;
...
#if 0
        ipl == SE_CUPIDO || ipl == SE_HADES || ipl == SE_ZEUS
            || ipl == SE_KRONOS || ipl == SE_APOLLON || ipl == SE_ADMETOS
            || ipl == SE_VULKANUS || ipl == SE_POSEIDON || ipl == SE_ISIS
            || ipl == SE_NEPTUNE_LEVERRIER || ipl == SE_NEPTUNE_ADAMS
#endif
            /* internal planet number */
            ipli = SEI_ANYBODY;
        pdp = &swed.pldat[ipli];
...
do_fict_plan:
        ...
        if (swi_osc_el_plan
            (tjd, pdp->x, ipl - SE_FICT_OFFSET, ipli, pedp->x, psdp->x,
             serr) != OK)
            goto return_error;
        if (retc == ERR)
            goto return_error;
...
swe_set_ephe_path(char *path)
{
    ...
    if (swi_trace_count < TRACE_COUNT_MAX) {
        if (swi_fp_trace_c != NULL) {
            ...
            fputs("  printf(\"swe_set_ephe_path: path_in = \");",
                  swi_fp_trace_c);
            fputs("  printf(s);\n", swi_fp_trace_c);
            fputs
...
swe_set_jpl_file(char *fname)
{
    ...
    if (swi_trace_count < TRACE_COUNT_MAX) {
        if (swi_fp_trace_c != NULL) {
            ...
            fputs("  printf(\"swe_set_jpl_file: fname_in = \");",
                  swi_fp_trace_c);
            fputs("  printf(s);\n", swi_fp_trace_c);
            fputs
                ("  printf(\"\\tfname_set = unknown to swetrace\\n\");  /* unknown to swetrace */\n",
                 swi_fp_trace_c);
            fflush(swi_fp_trace_c);
        }
        if (swi_fp_trace_out != NULL) {
 ...
 */
static void
calc_epsilon(double tjd, struct epsilon *e)
{
    e->teps = tjd;
 ...
 */
static int
main_planet(double tjd, int ipli, int32 epheflag, int32 iflag, char *serr)
{
    int retc;
    switch (epheflag) {
#ifndef NO_JPL
...
sweph_planet:
            ...
            retc =
                sweplan(tjd, ipli, SEI_FILE_PLANET, iflag, DO_SAVE, NULL,
                        NULL, NULL, NULL, serr);
            if (retc == ERR)
                return ERR;
            /* if sweph file not found, switch to moshier */
 ...
 */
static int
main_planet_bary(double tjd, int ipli, int32 epheflag, int32 iflag,
                 AS_BOOL do_save, double *xp, double *xe, double *xs,
                 double *xm, char *serr)
{
    int i, retc;
    switch (epheflag) {
...
sweph_planet:
            /* compute barycentric planet (+ earth, sun, moon) */
            retc =
                sweplan(tjd, ipli, SEI_FILE_PLANET, iflag, do_save, xp, xe,
                        xs, xm, serr);
#if 1
            if (retc == ERR || retc == NOT_AVAILABLE)
                return retc;
 ...
 */
static int
swemoon(double tjd, int32 iflag, AS_BOOL do_save, double *xpret, char *serr)
{
    int i, retc;
    ...
    if (tjd == pdp->teval && pdp->iephe == SEFLG_SWIEPH
        && (!speedf2 || speedf1)) {
        xp = pdp->x;
    }
    else {
        /* call sweph for moon */
        retc =
            sweph(tjd, SEI_MOON, SEI_FILE_MOON, iflag, NULL, do_save, xp,
                  serr);
        if (retc != OK)
            return (retc);
        if (do_save) {
 ...
 */
static int
sweplan(double tjd, int ipli, int ifno, int32 iflag, AS_BOOL do_save,
        ...
        char *serr)
{
    ...
    if (do_save || ipli == SEI_MOON || ipli == SEI_EARTH || xperet != NULL
        || xpmret != NULL)
        do_moon = TRUE;
    if (do_save) {
        xp = pdp->x;
    ...
    if (do_sunbary) {
        ...
        /* if planet has already been computed for this date, return
         * if speed flag has been turned on, recompute planet */
        if (tjd == psbdp->teval && psbdp->iephe == SEFLG_SWIEPH
            && (!speedf2 || speedf1)) {
            for (i = 0; i <= 5; i++)
                xps[i] = psbdp->x[i];
        }
            ...
            retc =
                sweph(tjd, SEI_SUNBARY, SEI_FILE_PLANET, iflag, NULL, do_save,
                      xps, serr);
             /**/ if (retc != OK)
                return (retc);
        }
    ...
    if (do_moon) {
        ...
        if (tjd == pmdp->teval && pmdp->iephe == SEFLG_SWIEPH
            && (!speedf2 || speedf1)) {
            for (i = 0; i <= 5; i++)
                xpm[i] = pmdp->x[i];
        }
            ...
            retc =
                sweph(tjd, SEI_MOON, SEI_FILE_MOON, iflag, NULL, do_save, xpm,
                      serr);
            if (retc == ERR)
                return (retc);
    ...
    if (do_earth) {
        ...
        if (tjd == pebdp->teval && pebdp->iephe == SEFLG_SWIEPH
            && (!speedf2 || speedf1)) {
            for (i = 0; i <= 5; i++)
                xpe[i] = pebdp->x[i];
        }
            ...
            retc =
                sweph(tjd, SEI_EMB, SEI_FILE_PLANET, iflag, NULL, do_save,
                      xpe, serr);
            if (retc != OK)
                return (retc);
 ...
 */
static int
jplplan(double tjd, int ipli, int32 iflag, AS_BOOL do_save, double *xpret,
        double *xperet, double *xpsret, char *serr)
{
    int i, retc;
    ...
    if (do_save || ipli == SEI_EARTH || xperet != NULL || (ipli == SEI_MOON))   /* && (iflag & (SEFLG_HELCTR | SEFLG_BARYCTR | SEFLG_NOABERR)))) */
        do_earth = TRUE;
    if (do_save || ipli == SEI_SUNBARY || xpsret != NULL || (ipli == SEI_MOON)) /* && (iflag & (SEFLG_HELCTR | SEFLG_NOABERR)))) */
        do_sunbary = TRUE;
    if (ipli == SEI_MOON)
        ictr = J_EARTH;
    /* open ephemeris, if still closed */
 ...
 */
static int
sweph(double tjd, int ipli, int ifno, int32 iflag, double *xsunb,
      AS_BOOL do_save, double *xpret, char *serr)
{
    ...
    speedf2 = iflag & SEFLG_SPEED;
    if (tjd == pdp->teval && pdp->iephe == SEFLG_SWIEPH
        && (!speedf2 || speedf1) && ipl < SEI_ANYBODY) {
        if (xpret != NULL)
            for (i = 0; i <= 5; i++)
                xpret[i] = pdp->x[i];
        return (OK);
...
again:
    ...
    if (tjd < fdp->tfstart || tjd > fdp->tfend) {
        if (serr != NULL) {
            if (tjd < fdp->tfstart)
                sprintf(s, "jd %f < Swiss Eph. lower limit %f;", tjd,
                        fdp->tfstart);
            else
                sprintf(s, "jd %f > Swiss Eph. upper limit %f;", tjd,
                        fdp->tfend);
   ...
   ******************************/
    ...
    for (i = 0; i <= 2; i++) {
        ...
        if (need_speed)
            xp[i + 3] =
                swi_edcheb(t, pdp->segp + (i * pdp->ncoe),
    ...
    if (ipl == SEI_SUNBARY && (pdp->iflg & SEI_FLG_EMBHEL)) {
        ...
        pedp->teval = 0;
        retc =
            sweph(tjd, SEI_EMB, ifno, iflag | SEFLG_SPEED, NULL, NO_SAVE,
                  xemb, serr);
        if (retc != OK)
            return (retc);
        pedp->teval = tsv;
 ...
 */
FILE *
swi_fopen(int ifno, char *fname, char *ephepath, char *serr)
{
    int np, i, j;
    FILE *fp = NULL;
    ...
    for (i = 0; i < np; i++) {
        ...
        if (strlen(fnamp) + strlen(fname) < AS_MAXCH)
            ...
            if (serr != NULL)
                sprintf(serr,
                        "error: file path and name must be shorter than %d.",
                        AS_MAXCH);
            return NULL;
        }
 ...
 */
static int
app_pos_etc_plan(int ipli, int32 iflag, char *serr)
{
   ...
   ************************************/
    ...
    if (iflag & SEFLG_TOPOCTR) {
        if (swed.topd.teval != pedp->teval || swed.topd.teval == 0) {
            if (swi_get_observer(pedp->teval, iflag, DO_SAVE, xobs, serr)
                != OK)
                return ERR;
        }
        else {
    ...
    if (!(iflag & SEFLG_TRUEPOS)) {
        ...
        switch (epheflag) {
            ...
            case SEFLG_SWIEPH:
                if (ibody == IS_PLANET)
                    retc =
                        sweplan(t, ipli, ifno, iflag, NO_SAVE, xx, xearth,
                                xsun, NULL, serr);
                else {          /*asteroid */
                    retc =
                        sweplan(t, SEI_EARTH, SEI_FILE_PLANET, iflag, NO_SAVE,
                                xearth, NULL, xsun, NULL, serr);
                    if (retc == OK)
                        retc =
                            sweph(t, ipli, ifno, iflag, xsun, NO_SAVE, xx,
                                  serr);
                }
                if (retc != OK)
                    return (retc);
            ...
            default:
                ...
                if (iflag & SEFLG_SPEED
                    ...
                    if (ibody == IS_PLANET)
                        retc =
                            swi_moshplan(t, ipli, NO_SAVE, xxsv, xearth,
                                         serr);
                    else {      /* if asteroid */
                        retc =
                            sweph(t, ipli, ifno, iflag, NULL, NO_SAVE, xxsv,
                                  serr);
                        if (retc == OK)
                            retc =
                                swi_moshplan(t, SEI_EARTH, NO_SAVE, xearth,
                                             xearth, serr);
                    }
                    if (retc != OK)
                        return (retc);
...
static int
app_pos_rest(struct plan_data *pdp, int32 iflag, double *xx, double *x2000,
    ...
    if (iflag & SEFLG_SIDEREAL) {
        ...
        if (swed.sidd.sid_mode & SE_SIDBIT_ECL_T0) {
            if (swi_trop_ra2sid_lon
                (x2000, pdp->xreturn + 6, pdp->xreturn + 18, iflag,
                 serr) != OK)
                return ERR;
...
swe_set_sid_mode(int32 sid_mode, double t0, double ayan_t0)
{
    ...
    if (sid_mode == SE_SIDM_J2000 || sid_mode == SE_SIDM_J1900
 ...
 */
int
swi_trop_ra2sid_lon(double *xin, double *xout, double *xoutr, int32 iflag,
                    char *serr)
{
    double x[6];
    int i;
 ...
 */
int
swi_trop_ra2sid_lon_sosy(double *xin, double *xout, double *xoutr,
 ...
 */
static int
app_pos_etc_plan_osc(int ipl, int ipli, int32 iflag, char *serr)
{
    int i, j, niter, retc;
    double xx[6], dx[3], dt, dtsave_for_defl;
    double xearth[6], xsun[6], xmoon[6];
    double xxsv[6], xxsp[3] = {
        0
    }, xobs[6], xobs2[6];
    double t;
    struct plan_data *pdp = &swed.pldat[ipli];
   ...
   *******************************/
    if (!(iflag & SEFLG_TRUEPOS)) {
        ...
        if (iflag & SEFLG_SPEED) {
            ...
            retc =
                main_planet_bary(t, SEI_EARTH, epheflag, iflag, NO_SAVE,
                                 xearth, xearth, xsun, xmoon, serr);
            if (swi_osc_el_plan
                (t, xx, ipl - SE_FICT_OFFSET, ipli, xearth, xsun, serr) != OK)
                return ERR;
            if (retc != OK)
                return (retc);
            if (iflag & SEFLG_TOPOCTR) {
 ...
 *		coordinates */
void
swi_precess_speed(double *xx, double t, int direction)
{
    struct epsilon *oe;
 ...
 */
void
swi_nutate(double *xx, int32 iflag, AS_BOOL backward)
{
    int i;
    double x[6], xv[6];
    for (i = 0; i <= 2; i++) {
        if (backward)
            x[i] =
                xx[0] * swed.nut.matrix[i][0] +
                xx[1] * swed.nut.matrix[i][1] + xx[2] * swed.nut.matrix[i][2];
        else
            x[i] =
                xx[0] * swed.nut.matrix[0][i] +
                xx[1] * swed.nut.matrix[1][i] + xx[2] * swed.nut.matrix[2][i];
    }
    if (iflag & SEFLG_SPEED) {
        /* correct speed:
         * first correct rotation */
        for (i = 0; i <= 2; i++) {
            if (backward)
                x[i + 3] =
                    xx[3] * swed.nut.matrix[i][0] +
                    xx[4] * swed.nut.matrix[i][1] +
                    xx[5] * swed.nut.matrix[i][2];
            else
                x[i + 3] =
                    xx[3] * swed.nut.matrix[0][i] +
        ...
        for (i = 0; i <= 2; i++) {
            if (backward)
                xv[i] =
                    xx[0] * swed.nutv.matrix[i][0] +
                    xx[1] * swed.nutv.matrix[i][1] +
                    xx[2] * swed.nutv.matrix[i][2];
            else
                xv[i] =
                    xx[0] * swed.nutv.matrix[0][i] +
 ...
 */
void
swi_aberr_light(double *xx, double *xe, int32 iflag)
{
    int i;
 ...
 */
void
swi_deflect_light(double *xx, double dt, int32 iflag)
{
    int i;
    double xx2[6];
    ...
    if (iflag & SEFLG_SPEED) {
        ...
        if (iephe == SEFLG_JPLEPH || iephe == SEFLG_SWIEPH) {
            for (i = 0; i <= 2; i++)
                e[i] =
                    xearth[i] - psdp->x[i] - dtsp * (xearth[i + 3] -
                                                     psdp->x[i + 3]);
        }
        else
            for (i = 0; i <= 2; i++)
        ...
        for (i = 0; i <= 2; i++)
            q[i] =
                u[i] + xearth[i] - xsun[i] - dtsp * (xearth[i + 3] -
                                                     xsun[i + 3]);
        ru = sqrt(square_sum(u));
        rq = sqrt(square_sum(q));
        re = sqrt(square_sum(e));
 ...
 */
static int
app_pos_etc_sun(int32 iflag, char *serr)
{
    int i, j, niter, retc = OK;
    int32 flg1, flg2;
                ...
                switch (pedp->iephe) {
                    ...
                    case SEFLG_JPLEPH:
                        if ((iflag & SEFLG_HELCTR)
                            || (iflag & SEFLG_BARYCTR))
                            retc =
                                swi_pleph(t, J_EARTH, J_SBARY, xearth, serr);
                        else
                            retc = swi_pleph(t, J_SUN, J_SBARY, xsun, serr);
                        if (retc != OK) {
                            swi_close_jpl_file();
                    ...
                    case SEFLG_SWIEPH:
                        ...
                        if ((iflag & SEFLG_HELCTR)
                            || (iflag & SEFLG_BARYCTR))
                            retc =
                                sweplan(t, SEI_EARTH, SEI_FILE_PLANET, iflag,
                                        NO_SAVE, xearth, NULL, xsun, NULL,
                                        serr);
                        else
                            retc =
                                sweph(t, SEI_SUNBARY, SEI_FILE_PLANET, iflag,
                                      NULL, NO_SAVE, xsun, serr);
                        break;
                    case SEFLG_MOSEPH:
                        if ((iflag & SEFLG_HELCTR)
                            || (iflag & SEFLG_BARYCTR))
 ...
 */
static int
app_pos_etc_moon(int32 iflag, char *serr)
{
    int i;
    int32 flg1, flg2;
   ...
   *******************************/
    if (iflag & SEFLG_TOPOCTR) {
        if (swed.topd.teval != pdp->teval || swed.topd.teval == 0) {
            if (swi_get_observer(pdp->teval, iflag, DO_SAVE, xobs, NULL)
                != OK)
                return ERR;
        }
        else {
   ...
   *******************************/
    if ((iflag & SEFLG_TRUEPOS) == 0) {
        ...
        switch (pdp->iephe) {
            ...
            case SEFLG_SWIEPH:
                retc =
                    sweplan(t, SEI_MOON, SEI_FILE_MOON, iflag, NO_SAVE, xx,
                            xe, xs, NULL, serr);
                if (retc != OK)
                    return (retc);
                for (i = 0; i <= 5; i++)
 ...
 */
static int
app_pos_etc_sbar(int32 iflag, char *serr)
{
    int i;
 ...
 */
static int
app_pos_etc_mean(int ipl, int32 iflag, char *serr)
{
    int i;
    int32 flg1, flg2;
    double xx[6], xxsv[6];
 ...
 */
static int
get_new_segment(double tjd, int ipli, int ifno, char *serr)
{
    ...
    retc =
        do_fread((void *)&fpos, 3, 1, 4, fp, fpos, freord, fendian, ifno,
                 serr);
    if (retc != OK)
        return (retc);
    ...
    for (icoord = 0; icoord < 3; icoord++) {
        ...
        retc =
            do_fread((void *)&c[0], 1, 2, 1, fp, SEI_CURR_FPOS, freord,
                     fendian, ifno, serr);
        if (retc != OK)
            return (retc);
        if (c[0] & 128) {
            nsizes = 6;
            retc =
                do_fread((void *)(c + 2), 1, 2, 1, fp, SEI_CURR_FPOS, freord,
                         fendian, ifno, serr);
            if (retc != OK)
                return (retc);
            nsize[0] = (int)c[1] / 16;
            ...
            nsize[5] = (int)c[3] % 16;
            nco =
                nsize[0] + nsize[1] + nsize[2] + nsize[3] + nsize[4] +
                nsize[5];
        }
        else {
            nsizes = 4;
        ...
        if (nco > pdp->ncoe) {
            if (serr != NULL) {
                sprintf(serr,
                        "error in ephemeris file: %d coefficients instead of %d. ",
                        nco, pdp->ncoe);
                if (strlen(serr) + strlen(fdp->fnam) < AS_MAXCH - 1)
                    sprintf(serr,
                            "error in ephemeris file %s: %d coefficients instead of %d. ",
        ...
        for (i = 0; i < nsizes; i++) {
            ...
            if (i < 4) {
                ...
                k = nsize[i];
                retc =
                    do_fread((void *)&longs[0], j, k, 4, fp, SEI_CURR_FPOS,
                ...
                for (m = 0; m < k; m++, idbl++) {
                    if (longs[m] & 1)   /* will be negative */
                        pdp->segp[idbl] =
                            -(((longs[m] + 1) / 2) / 1e+9 * pdp->rmax / 2);
                    else
                        pdp->segp[idbl] =
                            (longs[m] / 2) / 1e+9 * pdp->rmax / 2;
                }
            }
                ...
                k = (nsize[i] + 1) / 2;
                retc =
                    do_fread((void *)longs, j, k, 4, fp, SEI_CURR_FPOS,
                             freord, fendian, ifno, serr);
                if (retc != OK)
                    return (retc);
                for (m = 0, j = 0; m < k && j < nsize[i]; m++) {
                    for (n = 0, o = 16; n < 2 && j < nsize[i];
                         n++, j++, idbl++, longs[m] %= o, o /= 16) {
                        if (longs[m] & o)
                            pdp->segp[idbl] =
                                -(((longs[m] +
                                    o) / o / 2) * pdp->rmax / 2 / 1e+9);
                        else
                            pdp->segp[idbl] =
                                (longs[m] / o / 2) * pdp->rmax / 2 / 1e+9;
                    }
                }
            }
    ...
    if (ipli == SEI_SUNBARY) {
        ...
        for (i = 0; i < pdp->ncoe; i++)
            printf("%e, %e, %e\n", pdp->segp[i], pdp->segp[i + pdp->ncoe],
                   pdp->segp[i + 2 * pdp->ncoe]);
    }
#endif
    return (OK);
}
...
read_const(int ifno, char *serr)
{
   ...
   *************************************/
    ...
    if (strcmp(s2, s) != 0) {
        if (serr != NULL)
            sprintf(serr, "Ephemeris file name '%s' wrong; rename '%s' ", s2,
                    s);
        goto return_error;
    }
   ...
   *************************************/
    retc =
        do_fread((void *)&lng, 4, 1, 4, fp, SEI_CURR_FPOS, freord, fendian,
                 ifno, serr);
    if (retc != OK)
        return (retc);
    fpos = ftell(fp);
   ...
   **********************************************************/
    retc =
        do_fread((void *)&fdp->sweph_denum, 4, 1, 4, fp, fpos, freord,
                 fendian, ifno, serr);
    if (retc != OK)
        return (retc);
   ...
   *************************************/
    retc =
        do_fread((void *)&fdp->tfstart, 8, 1, 8, fp, SEI_CURR_FPOS, freord,
                 fendian, ifno, serr);
    if (retc != OK)
        return (retc);
    retc =
        do_fread((void *)&fdp->tfend, 8, 1, 8, fp, SEI_CURR_FPOS, freord,
                 fendian, ifno, serr);
    if (retc != OK)
        return (retc);
   ...
   *************************************/
    retc =
        do_fread((void *)&nplan, 2, 1, 2, fp, SEI_CURR_FPOS, freord, fendian,
                 ifno, serr);
    if (retc != OK)
        return (retc);
    if (nplan > 256) {
   ...
   *************************************/
    ...
    /* read CRC from file */
    retc =
        do_fread((void *)&ulng, 4, 1, 4, fp, SEI_CURR_FPOS, freord, fendian,
                 ifno, serr);
    if (retc != OK)
        return (retc);
   ...
   *************************************/
    ...
    retc =
        do_fread((void *)&doubles[0], 8, 5, 8, fp, SEI_CURR_FPOS, freord,
                 fendian, ifno, serr);
    if (retc != OK)
        return (retc);
   ...
   *************************************/
    for (kpl = 0; kpl < fdp->npl; kpl++) {
        ...
        retc =
            do_fread((void *)&pdp->lndx0, 4, 1, 4, fp, SEI_CURR_FPOS, freord,
                     fendian, ifno, serr);
        if (retc != OK)
            return (retc);
        /* flags: helio/geocentric, rotation, reference ellipse */
        retc =
            do_fread((void *)&pdp->iflg, 1, 1, sizeof(int32), fp,
                     SEI_CURR_FPOS, freord, fendian, ifno, serr);
        if (retc != OK)
            return (retc);
        ...
        retc =
            do_fread((void *)&pdp->ncoe, 1, 1, sizeof(int), fp, SEI_CURR_FPOS,
                     freord, fendian, ifno, serr);
        if (retc != OK)
            return (retc);
        /* rmax = normalisation factor */
        retc =
            do_fread((void *)&lng, 4, 1, 4, fp, SEI_CURR_FPOS, freord,
                     fendian, ifno, serr);
        if (retc != OK)
            return (retc);
        pdp->rmax = lng / 1000.0;
        ...
        retc =
            do_fread((void *)doubles, 8, 10, 8, fp, SEI_CURR_FPOS, freord,
                     fendian, ifno, serr);
        if (retc != OK)
            return (retc);
        ...
        if (pdp->iflg & SEI_FLG_ELLIPSE) {
            ...
            retc =
                do_fread((void *)pdp->refep, 8, 2 * pdp->ncoe, 8, fp,
                         SEI_CURR_FPOS, freord, fendian, ifno, serr);
            if (retc != OK)
                return (retc);
        }
 ...
 */
static int
do_fread(void *trg, int size, int count, int corrsize, FILE * fp, int32 fpos,
         int freord, int fendian, int ifno, char *serr)
{
    int i, j, k;
    int totsize;
    ...
    if (!freord && size == corrsize) {
        if (fread((void *)targ, (size_t) totsize, 1, fp) == 0) {
            if (serr != NULL) {
                ...
                if (strlen(serr) + strlen(swed.fidat[ifno].fnam) <
                    AS_MAXCH - 1)
                    sprintf(serr, "Ephemeris file %s is damaged.",
                            swed.fidat[ifno].fnam);
            }
            fclose(fp);
            fp = NULL;
            return (ERR);
 ...
 */
static void
embofs(double *xemb, double *xmoon)
{
    int i;
    for (i = 0; i <= 2; i++)
 ...
 */
static void
nut_matrix(struct nut *nu, struct epsilon *oe)
{
    double psi, eps;
 ...
 */
static int
lunar_osc_elem(double tjd, int ipl, int32 iflag, char *serr)
{
    int i, j, istart;
    int ipli = SEI_MOON;
    int32 epheflag = SEFLG_DEFAULTEPH;
    ...
    speedf2 = iflag & SEFLG_SPEED;
    if (tjd == ndp->teval && tjd != 0 && flg1 == flg2
        && (!speedf2 || speedf1)) {
        ndp->xflgs = iflag;
        ...
        case SEFLG_JPLEPH:
            ...
            for (i = istart; i <= 2; i++) {
                ...
                if ((iflag & SEFLG_TRUEPOS) == 0 && retc >= OK) {
                    ...
                    retc =
                        jplplan(t - dt, ipli, iflag, NO_SAVE, xpos[i], NULL,
                                NULL, serr);
                     /**/
                        /* read error or corrupt file */
                        if (retc == ERR)
...
sweph_moon:
            ...
            for (i = istart; i <= 2; i++) {
                if (i == 0)
                    ...
                    t = tjd;
                retc =
                    swemoon(t, iflag | SEFLG_SPEED, NO_SAVE, xpos[i], serr);
                 /**/ if (retc == ERR)
                    return (ERR);
                ...
                if ((iflag & SEFLG_TRUEPOS) == 0 && retc >= OK) {
                    dt = sqrt(square_sum(xpos[i])) * AUNIT / CLIGHT / 86400.0;
                    retc =
                        swemoon(t - dt, iflag | SEFLG_SPEED, NO_SAVE, xpos[i],
                                serr);
                     /**/ if (retc == ERR)
                        return (ERR);
                }
...
moshier_moon:
   ...
   ************************************************************/
    ...
    Gmsm =
        GEOGCONST * (1 +
                     1 / EARTH_MOON_MRAT) / AUNIT / AUNIT / AUNIT * 86400.0 *
        86400.0;
    /* three apogees */
    for (i = istart; i <= 2; i++) {
        /* node */
        ...
        sinE =
            1 / ecce / sqrt(sema * Gmsm) * dot_prod(xpos[i], (xpos[i] + 3));
        /* true anomaly */
   ...
   **********************************************************************/
    for (j = 0; j <= 1; j++) {
        ...
        swi_coortrf2(ndp->xreturn + 6, ndp->xreturn + 18, -oe->seps,
                     oe->ceps);
        if (iflag & SEFLG_SPEED)
            swi_coortrf2(ndp->xreturn + 9, ndp->xreturn + 21, -oe->seps,
                         oe->ceps);
#ifdef SID_TNODE_FROM_ECL_T0
...
#endif
        if (!(iflag & SEFLG_NONUT)) {
            swi_coortrf2(ndp->xreturn + 18, ndp->xreturn + 18, -swed.nut.snut,
                         swed.nut.cnut);
            if (iflag & SEFLG_SPEED)
                swi_coortrf2(ndp->xreturn + 21, ndp->xreturn + 21,
        ...
        if (iflag & SEFLG_SIDEREAL) {
            ...
            if ((swed.sidd.sid_mode & SE_SIDBIT_ECL_T0)
                ...
                if (swed.sidd.sid_mode & SE_SIDBIT_ECL_T0)
                    swi_trop_ra2sid_lon(x, ndp->xreturn + 6,
                                        ndp->xreturn + 18, iflag, NULL);
                /* project onto solar system equator */
                else if (swed.sidd.sid_mode & SE_SIDBIT_SSY_PLANE)
                    swi_trop_ra2sid_lon_sosy(x, ndp->xreturn + 6,
                                             ndp->xreturn + 18, iflag, NULL);
                /* to polar */
                swi_cartpol_sp(ndp->xreturn + 6, ndp->xreturn);
                swi_cartpol_sp(ndp->xreturn + 18, ndp->xreturn + 12);
            ...
            swi_cartpol_sp(ndp->xreturn + 18, ndp->xreturn + 12);
            swi_coortrf2(ndp->xreturn + 18, ndp->xreturn + 6,
                         swed.oec2000.seps, swed.oec2000.ceps);
            if (iflag & SEFLG_SPEED)
                swi_coortrf2(ndp->xreturn + 21, ndp->xreturn + 9,
                             swed.oec2000.seps, swed.oec2000.ceps);
            swi_cartpol_sp(ndp->xreturn + 6, ndp->xreturn);
        }
#endif
 ...
 */
static int
intp_apsides(double tjd, int ipl, int32 iflag, char *serr)
{
    int i;
    int32 flg1, flg2;
    struct plan_data *ndp;
   ...
   ************************************************************/
    ...
    if (iflag & SEFLG_SIDEREAL) {
        ...
        for (i = 0; i <= 5; i++)
            ndp->xreturn[18 + i] = x[i];
        swi_cartpol_sp(ndp->xreturn + 18, ndp->xreturn + 12);
        swi_coortrf2(ndp->xreturn + 18, ndp->xreturn + 6, swed.oec2000.seps,
                     swed.oec2000.ceps);
        if (iflag & SEFLG_SPEED)
            swi_coortrf2(ndp->xreturn + 21, ndp->xreturn + 9,
                         swed.oec2000.seps, swed.oec2000.ceps);
        ...
        if (iflag & SEFLG_SPEED)
            swi_coortrf2(ndp->xreturn + 21, ndp->xreturn + 9, oe->seps,
                         oe->ceps);
        if (!(iflag & SEFLG_NONUT)) {
            swi_coortrf2(ndp->xreturn + 6, ndp->xreturn + 6, nut->snut,
                         nut->cnut);
            if (iflag & SEFLG_SPEED)
                swi_coortrf2(ndp->xreturn + 9, ndp->xreturn + 9, nut->snut,
                             nut->cnut);
        }
        /* ecliptic polar */
        swi_cartpol_sp(ndp->xreturn + 6, ndp->xreturn);
 ...
 */
int
swi_plan_for_osc_elem(int32 iflag, double tjd, double *xx)
{
    int i;
    double x[6];
    struct nut nuttmp;
   ...
   ************************************************/
    if (!(iflag & SEFLG_NONUT)) {
        ...
        for (i = 0; i <= 2; i++)
            x[i] =
                xx[0] * nutp->matrix[0][i] + xx[1] * nutp->matrix[1][i] +
                xx[2] * nutp->matrix[2][i];
        /* speed:
         * rotation only */
        for (i = 0; i <= 2; i++)
            x[i + 3] =
                xx[3] * nutp->matrix[0][i] + xx[4] * nutp->matrix[1][i] +
...
meff(double r)
{
    ...
    if (r <= 0)
        ...
        return 1.0;
    for (i = 0; eff_arr[i].r > r; i++); /* empty body */
...
static void
denormalize_positions(double *x0, double *x1, double *x2)
{
    int i;
...
static void
calc_speed(double *x0, double *x1, double *x2, double dt)
{
    int i, j, k;
    double a, b;
    for (j = 0; j <= 18; j += 6) {
...
void
swi_check_nutation(double tjd, int32 iflag)
{
    int32 speedf1, speedf2;
    ...
    speedf2 = iflag & SEFLG_SPEED;
    if (!(iflag & SEFLG_NONUT)
        && (tjd != swed.nut.tnut || tjd == 0 || (!speedf1 && speedf2))) {
...
swe_fixstar(char *star, double tjd, int32 iflag, double *xx, char *serr)
{
   ...
   ******************************************************/
    if (swed.fixfp == NULL) {
        if ((swed.fixfp =
             swi_fopen(SEI_FILE_FIXSTAR, SE_STARFILE, swed.ephepath,
                       serr)) == NULL) {
            swed.is_old_starfile = TRUE;
            if ((swed.fixfp =
                 swi_fopen(SEI_FILE_FIXSTAR, SE_STARFILE_OLD, swed.ephepath,
                           NULL)) == NULL) {
                retc = ERR;
                goto return_err;
            }
    ...
    while (fgets(s, AS_MAXCH, swed.fixfp) != NULL) {
        ...
        if ((sp = strchr(s, ',')) == NULL) {
            if (serr != NULL)
                sprintf(serr, "star file %s damaged at line %d", SE_STARFILE,
                        fline);
            retc = ERR;
            goto return_err;
        }
...
found:
    ...
    if (i < 13) {
        if (serr != NULL)
            sprintf(serr, "data of star '%s,%s' incomplete", cpos[0],
                    cpos[1]);
        retc = ERR;
        goto return_err;
    }
   ...
   ****************************************/
    ...
    sinde = sin(de);
    x[3] =
        -ra_pm * cosde * sinra - de_pm * sinde * cosra +
        radv * parall * cosde * cosra;
    x[4] =
        ra_pm * cosde * cosra - de_pm * sinde * sinra +
   ...
   ****************************************************/
        if (!(iflag & SEFLG_BARYCTR)
            && (!(iflag & SEFLG_HELCTR) || !(iflag & SEFLG_MOSEPH))) {
            if ((retc =
                 main_planet(tjd, SEI_EARTH, epheflag, iflag, serr)) != OK) {
                /*retc = ERR;
                 * goto return_err; */
   ...
   **********************************/
    ...
    /* ICRS to J2000 */
    if (!(iflag & SEFLG_ICRS)
        && (swed.jpldenum >= 403 || iflag & SEFLG_BARYCTR)) {
        swi_bias(x, iflag, FALSE);
    }
     /**/
        /* save J2000 coordinates; required for sidereal positions */
...
int32 FAR PASCAL_CONV
swe_fixstar_ut(char *star, double tjd_ut, int32 iflag, double *xx, char *serr)
{
...
**********************************************************/
int32 FAR PASCAL_CONV
swe_fixstar_mag(char *star, double *mag, char *serr)
{
    int i;
    int star_nr = 0;
    AS_BOOL isnomclat = FALSE;
...
char *FAR PASCAL_CONV
swe_get_planet_name(int ipl, char *s)
{
    int i;
    int32 retc;
    double xp[6];
        ...
        default:
            ...
            if (ipl > SE_AST_OFFSET) {
                ...
                /* else try to get it from ephemeris file */
                    ...
                    retc =
                        sweph(J2000, ipl, SEI_FILE_ANY_AST, 0, NULL, NO_SAVE,
                              xp, NULL);
                    if (retc != ERR && retc != NOT_AVAILABLE)
                        strcpy(s, swed.fidat[SEI_FILE_ANY_AST].astnam);
                    else
                ...
                if (s[0] == '?' || isdigit((int)s[1])) {
                    ...
                    if ((fp =
                         swi_fopen(-1, SE_ASTNAMFILE, swed.ephepath,
                                   NULL)) != NULL) {
                        while (ipli != iplf
                               && (sp = fgets(si, AS_MAXCH, fp)) != NULL) {
                            while (*sp == ' ' || *sp == '\t' || *sp == '('
                                   || *sp == '[' || *sp == '{')
                                sp++;
                            if (*sp == '#' || *sp == '\r' || *sp == '\n'
...
char *FAR PASCAL_CONV
swe_get_ayanamsa_name(int32 isidmode)
{
...
#ifdef TRACE
static void
trace_swe_calc(int swtch, double tjd, int ipl, int32 iflag, double *xx,
               char *serr)
{
    if (swi_trace_count >= TRACE_COUNT_MAX)
        return;
    switch (swtch) {
        ...
        case 2:
            ...
            if (swi_fp_trace_out != NULL) {
                fprintf(swi_fp_trace_out,
                        "swe_calc: %f\t%d\t%d\t%f\t%f\t%f\t%f\t%f\t%f\t", tjd,
                        ipl, iflag, xx[0], xx[1], xx[2], xx[3], xx[4], xx[5]);
                if (serr != NULL && *serr != '\0') {
...
static void
trace_swe_fixstar(int swtch, char *star, double tjd, int32 iflag, double *xx,
                  char *serr)
{
    if (swi_trace_count >= TRACE_COUNT_MAX)
        return;
    switch (swtch) {
        case 1:
            if (swi_fp_trace_c != NULL) {
                ...
                fprintf(swi_fp_trace_c, " iflag = %d;\n", iflag);
                fprintf(swi_fp_trace_c,
                        "  iflgret = swe_fixstar(star, tjd, iflag, xx, serr);");
                fprintf(swi_fp_trace_c, "   /* xx = %d */\n", (int32) xx);
                fflush(swi_fp_trace_c);
            }
            break;
        case 2:
            if (swi_fp_trace_c != NULL) {
                fputs
                    ("  printf(\"swe_fixstar: %s\\t%f\\t%d\\t%f\\t%f\\t%f\\t%f\\t%f\\t%f\\t\", ",
                     swi_fp_trace_c);
                fputs
            ...
            if (swi_fp_trace_out != NULL) {
                fprintf(swi_fp_trace_out,
                        "swe_fixstar: %s\t%f\t%d\t%f\t%f\t%f\t%f\t%f\t%f\t",
                        star, tjd, iflag, xx[0], xx[1], xx[2], xx[3], xx[4],
                        xx[5]);
                if (serr != NULL && *serr != '\0') {
                    fputs(serr, swi_fp_trace_out);
                }
...
static void
trace_swe_get_planet_name(int swtch, int ipl, char *s)
{
    if (swi_trace_count >= TRACE_COUNT_MAX)
        return;
    switch (swtch) {
...
int
swi_get_observer(double tjd, int32 iflag, AS_BOOL do_save, double *xobs,
                 char *serr)
{
    int i;
 ...
 */
int FAR PASCAL_CONV
swe_time_equ(double tjd, double *E, char *serr)
{
    ...
    L0 = 280.4664567 + swe_degnorm(tau * 360007.6982779)
        + tau2 * 0.03032028 + tau3 * 1 / 49931 - tau4 * 1 / 15299 -
        tau5 * 1 / 1988000;
    swi_nutation(tjd, nutlo);
